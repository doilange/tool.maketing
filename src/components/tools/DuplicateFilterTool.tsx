...
import { Copy, Trash2, Check, Sparkles, AlertCircle } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold flex items-center mb-2 bg-clip-text text-transparent bg-brand-gradient dark:from-white dark:to-slate-300">
            <Sparkles className="w-8 h-8 mr-3 text-violet-500 dark:text-violet-400 animate-pulse-soft" />
            {t("title")}
          </h2>
        ...
        <div className="flex items-center gap-3">
          <button onClick={processDuplicates} className="bg-brand-gradient text-white hover:opacity-90 px-5 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95 shadow-md shadow-violet-500/15 cursor-pointer">
            {t("removeBtn")}
          </button>
          {duplicateCount > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-xl text-xs font-bold animate-in fade-in zoom-in-95 duration-200">
