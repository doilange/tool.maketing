...
import { Copy, Trash2, Code2, Check, Sparkles } from "lucide-react";
    ...
    <div className="container mx-auto px-4 py-8 select-none max-w-5xl space-y-6">
      ...
      <div>
        <h2 className="text-3xl font-extrabold flex items-center mb-2 bg-clip-text text-transparent bg-brand-gradient dark:from-white dark:to-slate-300">
          <Code2 className="w-8 h-8 mr-3 text-violet-500 dark:text-violet-400 animate-pulse-soft" />
          {t("title")}
        </h2>
      ...
      <div className="flex flex-wrap gap-4 items-end bg-white/40 dark:bg-[#0a1128]/45 p-4 rounded-2xl border border-white/10 dark:border-white/5 shadow-sm">
        ...
        <button onClick={processHtml} className="flex items-center bg-brand-gradient text-white hover:opacity-90 px-6 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95 shadow-md shadow-violet-500/15 h-9 cursor-pointer ml-auto">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between px-1">
                <label className="text-xs font-extrabold text-foreground">
                  {t("extractedUrls")} <span className="text-violet-500 font-mono">({output.split('\n').filter(Boolean).length})</span>
                </label>
